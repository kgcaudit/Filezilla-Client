    ...
    ExposedDropdownMenuBox(expanded = expanded, onExpandedChange = { expanded = it }) {
        ...
        ExposedDropdownMenu(expanded = expanded, onDismissRequest = { expanded = false }) {
            options.forEach { (value, text) ->
                // The one in force is marked, not merely echoed in the box
                // above: an open list of four encodings with nothing picked
                // out is a list the user has to remember their way through.
                val current = text == selected
                DropdownMenuItem(
                    text = {
                        Text(
                            text,
